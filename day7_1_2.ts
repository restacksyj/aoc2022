...
function looper(myhashmap:any ){
    ...
    for ( let j = 0; j < Object.values(myhashmap).length; j++){
        ...
        if (typeof val === "object") {
            let returnSum = looper(myhashmap[key])
...
// console.log(acc)
// console.log("here it is", directoriesSum)
...
for (let s = 0; s < Object.keys(finalHash).length; s++){
   ...
   for (let item of items){
       directoriesSum[key] += directoriesSum[item]
   }
